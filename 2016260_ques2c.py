#ACTUALLY ITS 2(B) for kfold BUT DOESN'T MATTER!
...
# np.random.shuffle(a)
# a.dump('./shuffled2.mat')
a= np.load('./shuffled2.mat')
b= len(x_train)//5
validationaccuracy= []
for i in range(5):
    validation_train= []
...
deviation= math.sqrt(deviation/len(validationaccuracy))
print ("The standard deviation is:- " + str(deviation))
print ("Clearly the accuracy is the highest for the fourth case.")
...
for i in range(5):
    ...
    for j in range(len(y_test)):
        if(y_pred[i][j]== 3 and validation_test[j]== 3):
            tp+= 1
        elif(y_pred[i][j]== 8 and validation_test[j]== 8):
            tn+= 1
        elif(y_pred[i][j]== 3 and validation_test[j]== 8):
            fp+= 1
        elif(y_pred[i][j]== 8 and validation_test[j]== 3):
            fn+= 1
